...
type PostViewProps = {
    ...
    likedPost: string[]
    LikePosts: (id: string, val: string, val2: string) => Promise<void>
    darkMode: boolean;
    commentSheetModalPopUp: () => void;
    user: any;
}

export const PostView = React.memo(({ user, commentSheetModalPopUp, darkMode, LikePosts, likedPost, OptionModalPopUp, EditViewImageModalPopUp, setCurrentStatus, currentUser, posts, setSelectedImageUrl, setPostUid, setSelectedPost }: PostViewProps) => {
                                        ...
                                        <TouchableOpacity key={index} className={darkMode ? "w-[100%] bg-[rgb(17,17,17)] shadow-sm pb-[0px] pt-[20px] h-[auto] my-[10px] rounded-2xl" : "w-[100%] bg-white shadow-sm pb-[0px] pt-[20px] h-[auto] my-[10px] rounded-2xl"} activeOpacity={.9} onPress={() => { setSelectedPost(item); setPostUid(item.id); setSelectedImageUrl(item.imgurl!); EditViewImageModalPopUp() }}>
                                            <View className="flex-row gap-[10] justify-start px-[6px] mb-[15px]">
                                                ...
                                                <View>
                                                    ...
                                                    <View className="flex-row gap-[10] items-center pb-[10px]">
                        ...
                        <View className="flex-1 pb-[100px]">
                            {
                                posts.map((item, index) => {
                                    const jsDate = item.createdAt ? item.createdAt instanceof Timestamp ? item.createdAt.toDate() : item.createdAt : undefined;
                                    return (
                                        <TouchableOpacity key={index} className={darkMode ? " px-[15px] pb-[20px] w-[100%] bg-[rgb(17,17,17)] shadow-sm pt-[20px] h-[auto] my-[10px] rounded-2xl" : "px-[15px] w-[100%] pb-[20px] bg-white shadow-sm  pt-[20px] h-[auto] my-[10px] rounded-2xl"} activeOpacity={.9} onLongPress={() => { setCurrentStatus(item?.des!); setPostUid(item.id); OptionModalPopUp() }}>
