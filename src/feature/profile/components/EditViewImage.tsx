...
import { PostType } from "../../../../context/GlobalContext";
type Props = {
...
export const EditViewImage = React.memo(({ editViewImageModal, EditViewImageModalPopOut, viewImage, setViewImage, selectedPost, HandlePostDelete }: Props) => {
    ...
    return (
        ...
        >
            <TouchableWithoutFeedback onPress={() => { Keyboard.dismiss(); setEditAction(false); optionPopOut() }}>
                <View className={editAction ? "flex-1 justify-start items-center bg-white absolute top-[0px] h-[100%] w-[100%]" : "flex-1 justify-center items-center bg-white absolute top-[0px] h-[100%] w-[100%]"} style={{ zIndex: viewImage ? 1 : -1 }}>
                    ...
                    <TouchableOpacity className=" absolute top-[30px] left-[5px] flex-row items-center justify-start w-[auto]" style={{ zIndex: viewImage ? 1 : -1 }} activeOpacity={0.6} onPress={() => { EditViewImageModalPopOut() }}>
                        ...
                        <Text className="text-[15px]" style={{ fontFamily: 'Poppins-SemiBold' }}>Back to profile</Text>
                    </TouchableOpacity>
                    ...
                    </TouchableOpacity>
                    {
                        editAction && (
                            <View className="w-[100%] mt-[100px] mb-[30px] px-[10px] h-[50px]">
                                <TextInput multiline numberOfLines={20} placeholder="Write caption..." className="font-[Poppins-Medium] w-[100%] h-[100%]" />
                            </View>
                        )
                    }
                    <Image resizeMode={'cover'} source={{ uri: selectedPost?.imgurl }} className="h-[300px] w-[100%]" />
                    <Animated.View className="justify-center items-center gap-[15] absolute h-[150px] w-[100%] bg-white shadow-transparent shadow-md rounded-t-3xl" style={{ bottom: bottomEditAnim }}>
